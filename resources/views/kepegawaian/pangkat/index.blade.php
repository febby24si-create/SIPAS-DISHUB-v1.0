<x-app-layout>
    <x-slot name="header">Daftar Kenaikan Pangkat</x-slot>
    ...
    <div style="display:flex; flex-direction:column; gap:20px;">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
            ...
            <a href="{{ route('kepegawaian.pangkat.create') }}" style="text-decoration:none; display:inline-flex; align-items:center; gap:8px; padding:10px 18px; background:linear-gradient(135deg,#1d4ed8,#3b82f6); color:white; border-radius:12px; font-size:13px; font-weight:600; white-space:nowrap;">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                Tambah Pengajuan Kenaikan Pangkat
            </a>
        </div>
            ...
            <table class="data-table" style="width:100%; border-collapse:collapse; text-align:left;">
                <thead>
                    <tr style="border-bottom:1px solid rgba(0,0,0,0.05); background:#f8fafc;">
                        ...
                        <th style="padding:16px 24px; font-size:12px; font-weight:600; color:#64748b; text-transform:uppercase;">Nomor Usulan</th>
                        <th style="padding:16px 24px; font-size:12px; font-weight:600; color:#64748b; text-transform:uppercase;">Pegawai</th>
                        <th style="padding:16px 24px; font-size:12px; font-weight:600; color:#64748b; text-transform:uppercase;">Kenaikan</th>
                        <th style="padding:16px 24px; font-size:12px; font-weight:600; color:#64748b; text-transform:uppercase;">TMT</th>
                ...
                <tbody>
                    @forelse ($pangkat as $item)
                        <tr style="border-bottom:1px solid rgba(0,0,0,0.05);">
                            <td style="padding:16px 24px; color:#94a3b8; font-size:13px;">{{ $pangkat->firstItem() + $loop->index }}.</td>
                            <td style="padding:16px 24px;">
                                <div style="font-family:monospace; font-size:13px; font-weight:600; color:#1e293b;">{{ $item->nomor_usulan ?? '-' }}</div>
                            </td>
                            <td style="padding:16px 24px;">
                            ...
                            <td style="padding:16px 24px; font-size:12px; color:#475569;">
                                <div style="display:flex; align-items:center; gap:4px; margin-bottom:4px;">
                                    <span style="color:#94a3b8;">Pangkat:</span>
                                    <span>{{ $item->pangkat_lama }}</span>
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:#94a3b8;"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                                    <span style="font-weight:600; color:#059669;">{{ $item->pangkat_baru }}</span>
                                </div>
                                <div style="display:flex; align-items:center; gap:4px;">
                                    <span style="color:#94a3b8;">Gol:</span>
                                    <span>{{ $item->golongan_lama }}</span>
                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:#94a3b8;"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                                    <span style="font-weight:600; color:#059669;">{{ $item->golongan_baru }}</span>
                                </div>
                            </td>
                            <td style="padding:16px 24px; font-size:13px; color:#475569;">
                                {{ $item->tmt?->format('d M Y') ?? '-' }}
                            </td>
                            <td style="padding:16px 24px;">
                                @php
                                    $statusColors = [
                    ...
                    @empty
                        <tr>
                            <td colspan="7" style="text-align:center; padding:48px 24px; color:#94a3b8;">
                                ...
                                <p style="margin:0; font-size:14px; font-weight:500;">Belum ada pengajuan kenaikan pangkat</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
