<x-app-layout>
    <x-slot name="header">Daftar Usulan Gaji Berkala</x-slot>
    ...
    <div style="display:flex; flex-direction:column; gap:20px;">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
            ...
            <a href="{{ route('kepegawaian.kgb.create') }}" style="text-decoration:none; display:inline-flex; align-items:center; gap:8px; padding:10px 18px; background:linear-gradient(135deg,#1d4ed8,#3b82f6); color:white; border-radius:12px; font-size:13px; font-weight:600; white-space:nowrap;">
            ...
            <table class="data-table" style="width:100%; border-collapse:collapse; text-align:left;">
                <thead>
                    <tr style="border-bottom:1px solid rgba(0,0,0,0.05); background:#f8fafc;">
                        ...
                        <th style="padding:16px 24px; font-size:12px; font-weight:600; color:#64748b; text-transform:uppercase;">Gaji Pokok</th>
                ...
                <tbody>
                    @forelse ($kgb as $item)
                        <tr style="border-bottom:1px solid rgba(0,0,0,0.05);">
                            <td style="padding:16px 24px; color:#94a3b8; font-size:13px;">{{ $kgb->firstItem() + $loop->index }}.</td>
                            ...
                            <td style="padding:16px 24px; font-size:12px; color:#475569;">
                                <div style="display:flex; align-items:center; gap:4px; margin-bottom:4px;">
                                    <span style="color:#94a3b8;">Lama:</span>
                                    <span>Rp {{ number_format($item->gaji_pokok_lama, 0, ',', '.') }}</span>
                                </div>
                                <div style="display:flex; align-items:center; gap:4px;">
                                    <span style="color:#94a3b8;">Baru:</span>
                                    <span style="font-weight:600; color:#059669;">Rp {{ number_format($item->gaji_pokok_baru, 0, ',', '.') }}</span>
                                </div>
                            </td>
                            <td style="padding:16px 24px; font-size:12px; color:#475569;">
                                <div style="display:flex; align-items:center; gap:4px; margin-bottom:4px;">
                                    <span style="color:#94a3b8;">Sblm:</span>
                                    <span>{{ $item->tmt_sebelumnya?->format('d M Y') }}</span>
                                </div>
                                <div style="display:flex; align-items:center; gap:4px;">
                                    <span style="color:#94a3b8;">Brkt:</span>
                                    <span style="font-weight:600; color:#059669;">{{ $item->tmt_berikutnya?->format('d M Y') }}</span>
                                </div>
                            </td>
                            <td style="padding:16px 24px;">
                                @php
                            ...
                            <td style="padding:16px 24px; text-align:right;">
                                <div style="display:flex; gap:8px; justify-content:flex-end;">
                                    <a href="{{ route('kepegawaian.kgb.show', $item) }}" title="Detail" style="font-size:12px; font-weight:600; color:#1d4ed8; text-decoration:none;">Detail</a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" style="text-align:center; padding:48px 24px; color:#94a3b8;">
                                ...
                                <p style="margin:0; font-size:14px; font-weight:500;">Belum ada pengajuan gaji berkala</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
