<x-app-layout>
    ...
    <div style="max-width:640px;">
        <form action="{{ route('surat-keluar.update', $surat) }}" method="POST" enctype="multipart/form-data"
            ...
            <div x-data="{ 
                    bidang: '{{ old('bidang_id', $surat->unitKerja?->parent_id ?? '') }}', 
                    seksi: '{{ old('unit_kerja_id', $surat->unit_kerja_id ?? '') }}',
                    bidangs: {{ $bidangs->toJson() }}
                }" style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
                
                <div>
                    <label style="display:block; font-size:13px; font-weight:600; color:#334155; margin-bottom:6px;">Bidang <span style="color:#94a3b8; font-weight:400;">(Opsional untuk surat lama)</span></label>
                ...
                <div>
                    <label style="display:block; font-size:13px; font-weight:600; color:#334155; margin-bottom:6px;">Seksi</label>
